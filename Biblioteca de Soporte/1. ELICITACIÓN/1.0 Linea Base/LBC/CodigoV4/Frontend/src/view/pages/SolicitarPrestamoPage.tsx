...
const SolicitarPrestamoPage: React.FC = () => {
  ...
  const handlePreSubmit = () => {
    if (!selectedEquipment) {
      toast({
        title: "Error",
        description: "Debe seleccionar un tipo de equipo",
        variant: "destructive",
      });
      return;
    }
    ...
    <AppSidebar>
      <div className="space-y-6 animate-fade-in relative">
        ...
        <div className="flex items-center justify-between">
          ...
          <div>
            <Button variant="espe" className='mx-2' onClick={handlePreSubmit} disabled={loading}>
              Solicitar
            </Button>
        ...
        <Dialog open={showConfirm} onOpenChange={setShowConfirm}>
          <DialogContent className="bg-card max-w-md">
            <DialogHeader>
              <DialogTitle className="text-foreground">Confirmar Préstamo</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="text-sm space-y-1 text-foreground">
                <p className="font-semibold">¿Estás seguro de solicitar este préstamo?</p>
                <div className="mt-2 p-3 bg-muted rounded text-xs text-muted-foreground">
                  <p><strong>Equipo:</strong> {selectedEquipment}</p>
                  <p><strong>Horario:</strong> {getSelectedTimeRange()}</p>
                </div>
              </div>
              <div className="flex justify-end gap-2">
                <Button variant="ghost" onClick={() => setShowConfirm(false)}>Cancelar</Button>
                <Button variant="espe" onClick={handleSubmit}>Confirmar</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
