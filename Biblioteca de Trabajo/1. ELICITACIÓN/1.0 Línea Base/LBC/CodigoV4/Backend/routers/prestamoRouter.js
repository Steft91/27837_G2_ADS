...
/**
 * @swagger
 * /api/prestamos/validate:
 *   post:
 *     summary: Validar préstamo por código (Técnico)
 *     tags: [Préstamos]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *     responses:
 *       200:
 *         description: Préstamo validado y activado
 *       400:
 *         description: Código inválido o estado incorrecto
 */
router.post('/validate', auth, authorize(['TECNICO', 'ADMIN']), async (req, res) => {
  try {
    const { code } = req.body;
    if (!code) throw new Error('Código es requerido');

    const prestamo = await service.validateCode(code);
    res.json({
      status: 'success',
      message: 'Préstamo validado exitosamente',
      content: prestamo
    });
  } catch (err) {
    res.status(400).json({
...
router.delete('/:id', auth, async (req, res) => {
  try {
    // Delegamos validación de permisos al servicio para tener lógica más robusta
    // Pasamos ID, Rol y UserId
    const anulado = await service.delete(req.params.id, req.user.role, req.user.id);
    res.json({
      status: 'success',
    ...
    // Si el error es de permisos o lógica de negocio, devolvemos 400 o 403
    // Por simplicidad, 400 para reglas de negocio
    res.status(400).json({
      status: 'error',
      message: err.message,
      content: null
